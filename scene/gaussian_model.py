...
class GaussianModel:
    ...
    def setup_functions(self):
        ...
        self.xyz_activation = lambda x: torch.clamp(x, min=-1+0.001, max=1-0.001)
        self.brdf_activation = lambda x: torch.clamp(x, min=1e-9, max=1.0)
        self.gamma_activation = lambda x: torch.clamp(x, min=1e-6, max=1.0)
    ...
    def get_material(self):
        return self.brdf_activation(self._pbr_features)

    @property
    def get_gamma(self):
        return self.gamma_activation(self.gamma)
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        self._pbr_features = nn.Parameter(materials.requires_grad_(True)) 
    ...
    def training_setup(self, training_args):
        ...
        l.extend([{
            'params': self.gamma,
            'lr': training_args.rotation_lr, # gamma_lr TODO
            'name': "gamma"},
        ])
    ...
    def save_ply(self, path):
        ...
        attributes = np.concatenate((xyz, normals, f_dc, f_rest, materials, opacities, scale, rotation), axis=1)
        elements[:] = list(map(tuple, attributes))
        el = PlyElement.describe(elements, 'vertex')
        PlyData([el]).write(path)

        root_path=os.path.dirname(path)
        other_atrributes_dict={"gamma": self.gamma,}
    ...
    def load_ply(self, path):
        ...
        material_names = [p.name for p in plydata.elements[0].properties if p.name.startswith("material_")]
        material_names = sorted(material_names, key = lambda x: int(x.split('_')[-1]))
        assert len(material_names) == 5
        materials = np.zeros((xyz.shape[0], len(material_names))) # [N,5]
        for idx, attr_name in enumerate(material_names):
            materials[:, idx] = np.asarray(plydata.elements[0][attr_name])
